...
// O nome SEM o prefixo que o SAP impõe ao tipo (`nomeacao.prefixo`; lock object: E) — é sobre isto
// que o guard-rail Z/Y roda. Tipo sem prefixo devolve o nome como veio. PURO.
export function semPrefixo(mod, name) {
  const n = String(name ?? '');
  const p = mod?.nomeacao?.prefixo;
  return p && n.toUpperCase().startsWith(p) ? n.slice(p.length) : n;
}
...
export function validarModulo(mod, arquivo = '?') {
  ...
  // Quase todo adtType tem subtipo (TABL/DT), mas nem todos: o RIS do s4h 758 devolve `AUTH` NU para
  // um campo de autorização (e `SUSO/B` para o objeto). Medido 2026-08-29 — por isso o subtipo é opcional.
  if (!/^[A-Z]{4}(\/[A-Z0-9]+)?$/.test(mod.adtType)) erro(`adtType "${mod.adtType}" não tem a forma TIPO ou TIPO/SUB`);
  if (mod.adtType !== mod.codigo && !mod.adtType.startsWith(mod.codigo + '/')) erro(`adtType "${mod.adtType}" não começa pelo codigo "${mod.codigo}"`);
  ...
  if (mod.forma === 'json') {
    const validas = ['nenhuma', 'mesmaSessao', 'sessaoNova'];
    if (!validas.includes(mod.ativacaoJson)) erro(`forma "json" exige ativacaoJson (${validas.join(' | ')})`);
  }
  if (mod.container) {
  ...
  // Tipo com nome imposto pelo contêiner (include de FUGR: L<GRUPO><SUFIXO>) põe o Z/Y no contêiner.
  if (mod.zyPeloContainer && !mod.container) erro('zyPeloContainer exige `container` — é dele que sai o nome que passa pelo Z/Y');
  const campoZY = mod.zyPeloContainer ? mod.container.param : 'name';
  // Prefixo imposto pelo SAP (lock object: E) sai antes do Z/Y — EYJBV_… é nosso, EMMARAE não.
  if (!/^\$?[ZY]/i.test(semPrefixo(mod, mod.exemplo.opts[campoZY]))) erro(`exemplo.opts.${campoZY} "${mod.exemplo.opts[campoZY]}" precisa ser objeto Z/Y${mod.nomeacao?.prefixo ? ` depois do prefixo ${mod.nomeacao.prefixo}` : ''}, ou pacote local $Z…/$Y… (o guard-rail vale para o exemplo)`);
  if (mod.nomeacao) {
    ...
    if (mod.nomeacao.prefixo !== undefined && (typeof mod.nomeacao.prefixo !== 'string' || !/^[A-Z]$/.test(mod.nomeacao.prefixo))) erro('nomeacao.prefixo precisa ser UMA letra maiúscula (a que o SAP impõe ao nome)');
    if (mod.nomeacao.prefixo && !String(mod.exemplo.opts.name).toUpperCase().startsWith(mod.nomeacao.prefixo)) erro(`exemplo.opts.name não começa pelo prefixo ${mod.nomeacao.prefixo} que o tipo declara`);
