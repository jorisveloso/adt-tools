...
import { validarModulo, montarRegistro, criarResolucao, normalizar, dicaDeErro, semPrefixo } from './_registro.mjs';
...
test('nomeacao.prefixo: o Z/Y roda depois do prefixo que o SAP impõe (lock object: E)', () => {
  // EZX_EX é nosso (E + Z…); o exemplo sem o prefixo declarado é recusado; prefixo tem de ser UMA letra.
  expect(() => validarModulo(base({ nomeacao: { max: 16, fonte: 'doc', prefixo: 'E' }, exemplo: { opts: { name: 'EZX_EX' } } }), 'table.mjs')).not.toThrow();
  expect(() => validarModulo(base({ nomeacao: { max: 16, fonte: 'doc', prefixo: 'E' }, exemplo: { opts: { name: 'ZX_EX' } } }), 'table.mjs')).toThrow(/prefixo E/);
  expect(() => validarModulo(base({ nomeacao: { max: 16, fonte: 'doc', prefixo: 'E' }, exemplo: { opts: { name: 'EMMARAE' } } }), 'table.mjs')).toThrow(/Z\/Y depois do prefixo E/);
  expect(() => validarModulo(base({ nomeacao: { max: 16, fonte: 'doc', prefixo: 'ez' } }), 'table.mjs')).toThrow(/UMA letra/);
  expect(semPrefixo({ nomeacao: { prefixo: 'E' } }, 'EYJBV_POC_LK')).toBe('YJBV_POC_LK');
  expect(semPrefixo({ nomeacao: { prefixo: 'E' } }, 'YJBV_POC_LK')).toBe('YJBV_POC_LK');
  expect(semPrefixo({}, 'YJBV_POC_LK')).toBe('YJBV_POC_LK');
});
